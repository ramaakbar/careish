<div>
    ...
    <div class="space-y-5">
        @forelse ($reviews as $review)
            <div class="flex flex-row justify-between w-full p-3 bg-white border rounded">
                <div>
                    ...
                    <div class="flex flex-col flex-1 space-y-2">
                        <div class="flex flex-col md:items-center md:space-x-4 md:flex-row">
                            <h3 class="text-xl font-semibold">{{ $review->transaction->nurse->name }}</h3>
                ...
                <x-dropdown>
                    ...
                    <x-dropdown.item label="Edit" wire:click="getReviewId({{ $review->id }})"
                        onclick="$openModal('reviewModal')" />
                    <x-dropdown.item label="Delete" wire:click="deleteConfirm({{ $review->id }})" />
                </x-dropdown>
        ...
        @empty
            <div class="grid items-center justify-center mt-10 space-y-5">
                <div class="w-full">
                    <svg xmlns="http://www.w3.org/2000/svg" data-name="Layer 1" viewBox="0 0 801.95277 537"
                        ...
                        <path
                            ...
                            transform="translate(-199.02361 -181.5)" fill="#504be5" />
                        <path
                ...
                <p class="text-gray-600">You haven't reviewed any nurse yet</p>
            </div>
        @endforelse
    ...
    <x-modal.card title="Update Review" wire:model.defer="reviewModal">
        @if ($show)
            <div class="px-2">
                <form id="reviewForm" wire:submit.prevent="submit">
                    <div class="mb-4">
                        <div class="flex items-center"><img class="w-8 h-8 mr-2 rounded-full"
                                src="{{ asset('/storage/' . $reviewModel->transaction->nurse->picture) }}"
                                alt="{{ $reviewModel->transaction->nurse->name . ' picture' }}" alt="user photo">
                            <span class="text-gray-600">{{ $reviewModel->transaction->nurse->name }}</span>
                        </div>
                    </div>
                    <div class="mb-4">
                    ...
                    <button
                        class="w-full text-white bg-gray-800 hover:bg-gray-900 focus:outline-none focus:ring-4 focus:ring-gray-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 ">Update
                        Review</button>
                </form>
