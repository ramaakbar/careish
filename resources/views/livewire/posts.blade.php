<div>
    <section class="px-5 mx-auto scroll-m-16 max-w-7xl">
        <div class="max-w-lg mx-auto mb-10 text-center">
            ...
            <div class="flex space-x-4">
                ...
                <select wire:model="category"
                    class="bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 w-1/4 p-2.5"
                    id="">
        ...
        <div wire:loading.delay class="min-h-[60vh] flex items-center justify-center w-full">
            <p class="flex items-center justify-center w-full">Loading...</p>
        </div>
        ...
        @if ($posts->count())
            ...
            <a href="/articles/{{ $posts[0]->id }}" wire:loading.remove class="block mb-8">
                ...
                <div class="mt-3 space-y-1">
                    <p class="text-gray-600 ">{{ Carbon\Carbon::parse($posts[0]->created_at)->format('M d, Y') }}</p>
                    <h3 class="text-xl font-bold">{{ $posts[0]->title }}</h3>
                    @if ($posts[0]->postCategoriesId == 0)
                        <span
                            class="bg-blue-100 text-blue-800 text-xs font-semibold mr-2 px-2.5 py-0.5 rounded">{{ $posts[0]->category }}</span>
                    @elseif ($posts[0]->postCategoriesId == 1)
                        <span
                            class="bg-pink-100 text-pink-800 text-xs font-semibold mr-2 px-2.5 py-0.5 rounded">{{ $posts[0]->category }}</span>
                    @else
                        <span
                            class="bg-indigo-100 text-indigo-800 text-xs font-semibold mr-2 px-2.5 py-0.5 rounded">{{ $posts[0]->category }}</span>
                    @endif
                ...
                @foreach ($posts->skip(1) as $post)
                    <a href="/articles/{{ $post->id }}" class="block">
                        ...
                        <div class="mt-3 space-y-1">
                            ...
                            <h3 class="text-xl font-bold truncate">{{ $post->title }}</h3>
                            @if ($post->postCategoriesId == 0)
                                <span
                                    class="bg-blue-100 text-blue-800 text-xs font-semibold mr-2 px-2.5 py-0.5 rounded">{{ $post->category }}</span>
                            @elseif ($post->postCategoriesId == 1)
                                <span
                                    class="bg-pink-100 text-pink-800 text-xs font-semibold mr-2 px-2.5 py-0.5 rounded">{{ $post->category }}</span>
                            @else
                                <span
                                    class="bg-indigo-100 text-indigo-800 text-xs font-semibold mr-2 px-2.5 py-0.5 rounded">{{ $post->category }}</span>
                            @endif
                        </div>
                    </a>
                @endforeach
        ...
        @else
            <div wire:loading.remove
                class="flex flex-col items-center justify-center p-8 mx-auto space-y-4 text-center">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-56" data-name="Layer 1"
                    viewBox="0 0 808.84067 681.8936" xmlns:xlink="http://www.w3.org/1999/xlink">
                    <path d="M866.24553,659.31387,597.90045,536.80922,736.07526,234.13885l268.34507,122.50466Z"
                        transform="translate(-195.57967 -109.0532)" fill="#f2f2f2" />
                    <path
                        ...
                        transform="translate(-195.57967 -109.0532)" fill="#fff" />
                    <path
                    ...
                    <path
                        ...
                        transform="translate(-195.57967 -109.0532)" fill="#fff" />
                    <path d="M320.27065,356.64351,588.61572,234.13885,726.79053,536.80922,458.44546,659.31387Z"
                        transform="translate(-195.57967 -109.0532)" fill="#f2f2f2" />
                    <path
                    ...
                    <path
                        ...
                        transform="translate(-195.57967 -109.0532)" fill="#f0f0f0" />
                    <path
                    ...
                    <path
                        ...
                        transform="translate(-195.57967 -109.0532)" fill="#6c63ff" />
                    <path
                        ...
                        transform="translate(-195.57967 -109.0532)" fill="#3f3d56" />
                    <path
                        ...
                        transform="translate(-195.57967 -109.0532)" fill="#ffb6b6" />
                    <path
                        ...
                        transform="translate(-195.57967 -109.0532)" fill="#e4e4e4" />
                    <polygon points="203.799 669.082 216.058 669.082 221.891 621.794 203.796 621.794 203.799 669.082"
                        fill="#ffb6b6" />
                    <path
                        d="M396.25093,774.13262l24.144-.001h.001a15.3873,15.3873,0,0,1,15.38647,15.38623v.5l-39.53076.00146Z"
                        transform="translate(-195.57967 -109.0532)" fill="#2f2e41" />
                    <polygon points="134.799 669.082 147.058 669.082 152.891 621.794 134.796 621.794 134.799 669.082"
                        fill="#ffb6b6" />
                    <path
                    ...
                    <path
                        ...
                        transform="translate(-195.57967 -109.0532)" fill="#3f3d56" />
                    <path d="M577.57967,790.2756h-381a1,1,0,1,1,0-2h381a1,1,0,0,1,0,2Z"
                        transform="translate(-195.57967 -109.0532)" fill="#cacaca" />
                    <path
